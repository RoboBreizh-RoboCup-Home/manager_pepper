...
template <typename T>
constexpr auto user_serialize_fn<std::optional<T>, std::enable_if_t<user_serialize_fn<T> != nullptr>> =
    [](const std::optional<T>& arg) { return (arg ? std::make_optional(user_serialize_fn<T>(*arg)) : std::nullopt); };
...
inline void createFunction(T fn)
{
  ...
  auto wrapper_fn = [](sqlite3_context* context, int argc, sqlite3_value** argv) {
    ...
    using arg_types_decayed = typename detail::decay_tuple_args<typename fn_info::arg_types>::type;
    ...
    auto value_dispatcher = [argv, &idx](auto& arg, auto& self) {
      ...
      else if constexpr (std::is_same_v<std::string, arg_t> || std::is_same_v<std::string_view, arg_t>)
      {
        auto ptr = (const char*)sqlite3_value_text(argv[idx]);
        auto len = sqlite3_value_bytes(argv[idx]);
        arg = arg_t(ptr, len);
      }
      else if constexpr (std::is_same_v<sqlite::blob, arg_t> || std::is_same_v<sqlite::blob_view, arg_t>)
      {
    ...
    std::apply([&value_dispatcher](auto&... args) { (value_dispatcher(args, value_dispatcher), ...); }, arg_tuple);
    ...
    auto result_dispatcher = [context](auto&& res, auto& self) {
      ...
      else if constexpr (std::is_same_v<std::string, res_t> || std::is_same_v<sqlite::blob, res_t>)
      {
        ...
        {
          sqlite3_result_text(context, saved_str->data(), saved_str->size(), destructor);
        }
        else
        {
          sqlite3_result_blob(context, saved_str->data(), saved_str->size(), destructor);
        }
      }
    ...
    result_dispatcher(std::apply(saved_fn, std::move(arg_tuple)), result_dispatcher);
  };

  detail::function_creation_hooks.emplace_back([wrapper_fn](sqlite3* db_handle) {
    sqlite3_create_function(db_handle, fn_name, fn_info::num_args, SQLITE_UTF8 | SQLITE_DETERMINISTIC, nullptr,
...
private:
    ...
    Connection(void)
    {
      ...
      do
      {
        ...
        if (!detail::sqlite3_configured)
        {
          ...
          sqlite3_config(SQLITE_CONFIG_LOG, detail::sqlite_error_log_callback, nullptr);
  ...
  private:
    PreparedStmtCache(void) : db_handle(connection_tls().db_handle), first_free_stmt(nullptr), other_free_stmts()
    {
    }
    ...
    sqlite3_stmt* internal_get(void)
    {
      ...
      else
      {
        ...
        auto ret = sqlite3_prepare_v3(db_handle, query_str_view.data(), query_str_view.length() + 1,
...
public:
  ...
  static QueryResult query(Ts&&... bind_args)
  {
    ...
    else
    {
      ...
      auto bind_dispatcher = [stmt, &idx](const auto& arg, auto& self) {
        ...
        else if constexpr (std::is_same_v<const char*, arg_t> || std::is_same_v<char*, arg_t>)
        {
        ...
        else if constexpr (std::is_same_v<blob, arg_t> || std::is_same_v<blob_view, arg_t>)
        {
  ...
  private:
    ...
    QueryResult(sqlite3_stmt* stmt_, PutCallbackType* put_cb_) : stmt(stmt_), put_cb(put_cb_)
    {
      ret = sqlite3_step(stmt);
    }
