...
NavigationPlace NavigationModel::getLocationFromName(std::string location_name)
{
  ...
  while ((rc = sqlite3_step(pStmt)) == SQLITE_ROW)
  {
     ...
     * np.name,np.frame,np.pose.position.x,np.pose.position.y,np.pose.position.z,np.pose.orientation.w,np.pose.orientation.x,np.pose.orientation.y,np.pose.orientation.z);
     * /1* 3 *1/ */
  }
  return np;
}
