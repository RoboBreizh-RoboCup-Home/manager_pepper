...
Person VisionModel::selectFirstPerson()
{
  ...
  query =
      "SELECT person.name, person.favorite_drink, person.gender, person.age, color_cloth.label as cloth_color_id, "
      "color_skin.label as skin_color_id FROM person LEFT JOIN color color_cloth ON person.cloth_color_id = "
      "color_cloth.id LEFT JOIN color color_skin ON person.skin_color_id = color_skin.id order by person.id ASC limit "
      "1";
  pStmt = nullptr;
  int rc;
  Person person;
...
void VisionModel::updateFirstPerson(Person person)
{
  ...
  query =
      "UPDATE person SET gender = (?), age = (?), cloth_color_id = (?), skin_color_id = (?), posture = (?), height = "
      "(?), position_x = (?), position_y = (?),position_z = (?), distance = (?) WHERE id = (?)";
  pStmt = nullptr;
  int rc;
...
{
  query =
      "SELECT object.label, obj_color.label as color_id, object.position_x, object.position_y, object.position_z, "
      "object.distance FROM object LEFT JOIN color obj_color ON object.color_id = obj_color.id order by object.id DESC "
      "limit 1";
  pStmt = nullptr;
  int rc;
  int id = -1;
...
Person VisionModel::selectLastPerson()
{
  ...
  query =
      "SELECT person.name, person.favorite_drink, person.gender, person.age, color_cloth.label as cloth_color_id, "
      "color_skin.label as skin_color_id FROM person LEFT JOIN color color_cloth ON person.cloth_color_id = "
      "color_cloth.id LEFT JOIN color color_skin ON person.skin_color_id = color_skin.id order by person.id DESC limit "
      "1";
  pStmt = nullptr;
  int rc;
  Person person;
...
void VisionModel::createPersonFromFeatures(std::string gender, std::string age, std::string cloth_color,
...
void VisionModel::createPerson(Person person)
{
  ...
  query =
      "INSERT INTO person (gender, age,cloth_color_id, "
      "skin_color_id,posture,height,position_x,position_y,position_z,distance) VALUES (?,?,?,?,?,?,?,?,?,?)";
  pStmt = nullptr;
  int rc;
...
std::vector<robobreizh::Object> VisionModel::getAllObject()
{
  query =
      "SELECT object.label, obj_color.label as color_id, object.position_x, object.position_y, object.position_z, "
      "object.distance FROM object LEFT JOIN color obj_color ON object.color_id = obj_color.id";
  pStmt = nullptr;
  int rc;
  int id = -1;
...
std::vector<robobreizh::Object> VisionModel::getObjectsByLabel(std::string label)
{
  query =
      "SELECT object.label, obj_color.label as color_id, object.position_x, object.position_y, object.position_z, "
      "object.distance FROM object LEFT JOIN color obj_color ON object.color_id = obj_color.id where object.label = "
      "(?)";
  pStmt = nullptr;
  int rc;
  int id = -1;
...
{
  query =
      "SELECT person.name, person.favorite_drink, person.gender,person.age, color_skin.label as skin_color_id, "
      "color_cloth.label as cloth_color_id, person.posture, person.height, person.position_x, person.position_y, "
      "person.position_z, person.distance FROM person LEFT JOIN color color_cloth ON person.cloth_color_id = "
      "color_cloth.id LEFT JOIN color color_skin ON person.skin_color_id = color_skin.id";
  pStmt = nullptr;
  int rc;
  int id = -1;
