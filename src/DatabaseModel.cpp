...
std::vector<Person> DialogModel::getSeatedPerson() {
  query =
      ...
      "color_skin.label as skin_color_id FROM seated_person LEFT JOIN person ON seated_person.person_id = person.id "
      "LEFT JOIN color color_cloth ON person.cloth_color_id = color_cloth.id LEFT JOIN color color_skin ON "
      "person.skin_color_id = color_skin.id order by person.id";
  pStmt = nullptr;
  int rc;
...
Person DialogModel::getLastPersonWithName() {
  query =
      ...
      "color_cloth.id LEFT JOIN color color_skin ON person.skin_color_id = color_skin.id WHERE name IS NOT NULL order "
      "by person.id DESC limit 1";
  pStmt = nullptr;
  int rc;
  Person person;
