    ...
    for (int i = 0; i < nbPersons; i++)
    {
      ...
      if ((float)pers.distance < distMax)
      {
        ...
        ROS_INFO(
            ...
            "position (%f,%f,%f)",
            i, person.cloth_color.c_str(), person.age.c_str(), person.gender.c_str(), person.skin_color.c_str(),
            person.posture.c_str(), person.height, person.distance, person.pos_x, person.pos_y, person.pos_z);
      }
    }
  ...
  if (client.call(srv))
  {
    ...
    for (geometry_msgs::Pose pose : poseArray.poses)
    {
      ...
      geometry_msgs::Point coord =
          convertOdomToMap((float)pose.position.x, (float)pose.position.y, (float)pose.position.z);
      person.pos_x = coord.x;
      person.pos_y = coord.y;
      person.pos_z = coord.z;

      ROS_INFO("...got personne %s position (%f,%f,%f)", person.posture.c_str(), person.pos_x, person.pos_y,
               person.pos_z);
...
bool findHumanAndStoreFeatures(robobreizh::Person* person)
{
  ...
  if (client.call(srv))
  {
    ...
    std::cout << person->age << ", " << person->cloth_color << ", " << person->gender << ", " << person->skin_color <<
    std::endl;*/
...
{
  vector<std::string> fruits{ "Apple",      "Fruit",       "Grape",      "Tomato",     "Lemon",
                              "Banana",     "Orange",      "Coconut",    "Mango",      "Pineapple",
                              "Grapefruit", "Pomegranate", "Watermelon", "Strawberry", "Peach",
                              "Cantaloupe", "apple",       "banana",     "orange" };
  vector<std::string> vegetables{ "carrot", "broccoli" };
  vector<std::string> otherFood{
    "Food",      "Croissant", "Doughnut",     "Hot dog",     "Fast food", "Popcorn", "Cheese",    "Muffin",
    "Cookie",    "Dessert",   "French fries", "Baked goods", "Pasta",     "Pizza",   "Sushi",     "Bread",
    "Ice cream", "Salad",     "Sandwich",     "Pastry",      "Waffle",    "Pancake", "Burrito",   "Snack",
    "Taco",      "Hamburger", "Cake",         "Honeycomb",   "Pretzel",   "Bagel",   "Guacamole", "Submarine sandwich",
    "sandwich",  "hot dog",   "pizza",        "donut",       "cake",      "Candy"
  };
...
/*bool waitForHuman()
{
  ...
  ros::ServiceClient client =
nh.serviceClient<perception_pepper::object_detection_service>("/robobreizh/perception_pepper/object_detection_service");
